...
function CardRating({ rating, startDate, endDate }) {
    ...
    for (let i = 0; i < 5; i += 1) {
      ...
      arrStars.push(
        <div className="card-rating__star card-rating__star_out" key={i}>
          <div className="card-rating__star card-rating__star_in" style={{ width: `${paintStar(widthStar)}%` }} />
        </div>
      )
      widthStar -= 1;
    }
  ...
  return (
    ...
    <Link Link to={'/'} className="card-rating" >
      <div className="card-rating__stars-container">
        {setStars()}
      </div>
      <h3 className="card-rating__title">Рейтинг за неделю</h3>
      <p className="card-rating__subtitle">
        {formatText()}
      </p>
